...
def savePost():
    post = {"title" : request.args.get("title"),"uploader":"temp","time":"temp", "content" : request.stream.read().decode('utf-8'),"images":[]}
    client = MongoClient('mongodb://127.0.0.1:27017')
    db = client.week2db
    collection = db.posts
    try:
...
def saveImage():
    ...
    try:
        collection.update({"id":request.args.get('id'),
            {$push: [{"image_name":request.args.get('name'),
                        "image":request.stream.read().decode('utf-8')}]}})
    except:
